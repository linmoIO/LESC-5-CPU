...
class PiplineCPUwithBPandFSimplify(
    instFile: String = "",
    dataFile: String = "",
    startAddress: Int = 0x0
) extends Module {
...
object PiplineCPUwithBPandFSimplify {
  def main(args: Array[String]) = {
    print(
      getVerilogString(new PiplineCPUwithBPandFSimplify())
    )
    emitVerilog(
      new PiplineCPUwithBPandFSimplify(),
      Array(
        "--target-dir",
        s"generated/${PiplineCPUwithBPandFSimplify.toString().split('$')(0).split('.').last}"
      )
    )
    print("\n[Success]\n")
  }
